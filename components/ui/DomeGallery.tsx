...
function buildItems(pool: DomeImage[], seg: number) {
  ...
  const usedImages: DomeImage[] = Array.from({ length: totalSlots }, (_, i) => normalizedImages[i % normalizedImages.length])
...
export default function DomeGallery({
  ...
  const items = useMemo(
    () => buildItems(images as DomeImage[], segments),
    [images, segments]
  )
  ...
  const startInertia = useCallback(
    (vxRaw: number, vyRaw: number) => {
      const MAX_V = 1.4
      let vX = clamp(vxRaw, -MAX_V, MAX_V) * 80
      let vY = clamp(vyRaw, -MAX_V, MAX_V) * 80
      let frames = 0
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const cleanup = () => {
        ...
        if (refDiv) (refDiv as HTMLElement).remove()
