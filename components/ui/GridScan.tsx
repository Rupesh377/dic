...
// Shader source adapted from React Bits grid-scan background
const frag = `
...
export function GridScan({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // simple damp
      const lerpFactor = 1 - Math.exp(-dt / smoothTime)
      lookCurrent.current.lerp(lookTarget.current, lerpFactor)
      tiltCurrent.current += (tiltTarget.current - tiltCurrent.current) * lerpFactor
      yawCurrent.current += (yawTarget.current - yawCurrent.current) * lerpFactor
  ...
  }, [lineThickness, linesColor, scanColor, gridScale, bloomIntensity, chromaticAberration, noiseIntensity, scanOpacity])

  return <div ref={containerRef} className={`gridscan-root${className ? ` ${className}` : ''}`} style={style} />
}
