...
function extendMaterial(BaseMaterial: any, cfg: any) {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    ;(uniforms as any)[key] =
      u !== null && typeof u === 'object' && 'value' in (u as any) ? u : { value: u }
  })
...
// Scene that lives inside the R3F <Canvas>
const BeamsScene = ({
  material,
  beamWidth,
  beamHeight,
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent) => {
      ...
      // small tilt so it feels responsive but not too strong
      targetRot.current.x = -ny * 0.35
      targetRot.current.y = nx * 0.5
    }
  ...
  useFrame(() => {
    ...
    if (!group) return
    const baseZ = degToRad(rotation)
    const ease = 0.06
    group.rotation.x += (targetRot.current.x - group.rotation.x) * ease
    group.rotation.y += (targetRot.current.y - group.rotation.y) * ease
...
const Beams = ({
  ...
  // material still memoized once per Beams instance for performance
