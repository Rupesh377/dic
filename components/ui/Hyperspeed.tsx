...
type HyperspeedEffectOptions = typeof DEFAULT_EFFECT_OPTIONS & {
...
const DEFAULT_EFFECT_OPTIONS = {
  onSpeedUp: () => {},
  onSlowDown: () => {},
  distortion: 'turbulentDistortion' as DistortionKey,
  length: 400,
  roadWidth: 10,
  islandWidth: 2,
  lanesPerRoad: 4,
...
// Helper used inside the effect (original implementation)
...
const Hyperspeed = ({ effectOptions = hyperspeedPresets.one }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    // Cleanup previous instance if we re-render with new options
    if (appRef.current) {
    ...
    // The following is adapted from the original Hyperspeed implementation on reactbits.dev
    // (trimmed only where necessary to fit into a component-based setup)
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          const finalLaneX = laneX + carShiftX

          const offsetY = random(options.carFloorSeparation) + radius * 1.3
          const offsetZ = -random(options.length)

          aOffset.push(finalLaneX - carWidth / 2, offsetY, offsetZ)
          aOffset.push(finalLaneX + carWidth / 2, offsetY, offsetZ)
